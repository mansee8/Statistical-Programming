...
def train(X, Y, Xtest, Ytest, m=100, num_iterations=4000,
    ...
    sigma = 0.1
    beta = np.multiply(np.random.rand(p+1,1), sigma)
    beta = beta.reshape((p+1,1))
    for it in xrange(int(num_iterations)):
        expon = np.exp(np.multiply(-1, np.dot(X1, beta)))
        train_pr = np.divide(1, (1+expon))
        r = Y - train_pr
        repeat = np.repeat(r, p, axis = 1)
        dbeta = np.dot(np.ones((1,n)), (np.multiply(r, np.divide(X1, n))))
        beta = beta + np.multiply(learning_rate, np.transpose(dbeta))
        
        expon2 = np.exp(np.multiply(-1, np.dot(X1test, beta)))
        test_pr = np.divide(1, (1+expon2))
        
    accuracies1 = np.vstack((accuracies1, accuracy(train_pr, Y)))
    accuracies2 = np.vstack((accuracies2, accuracy(test_pr, Ytest)))
    return beta, accuracies1, accuracies2
...
beta, train_accuracy, testing_accuracy = train(training_digits, training_labels, testing_digits, testing_labels)
